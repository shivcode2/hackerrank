...
int birthdayCakeCandles(vector<int> candles) {
    vector<int>biggest_Candle(2);  //Vector == {a,b}, a == size of biggest candle; b == count of biggest candle.
    
    for(int i=0; i<candles.size(); i++)
    {
        if(biggest_Candle[0] == candles[i])
            biggest_Candle[1]++;
        else if(biggest_Candle[0] < candles[i])
        {
            biggest_Candle[0] = candles[i];
            biggest_Candle[1] = 1;
        }
    }
    return biggest_Candle[1];
}
